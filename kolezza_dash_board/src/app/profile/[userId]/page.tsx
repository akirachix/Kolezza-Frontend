      ...
      <div className="flex-1 bg-white p-10 overflow-y-auto">
        ...
        <div className="flex items-center mb-8 justify-center relative">
          <div className="bg-[#90BD31] rounded-full w-[280px] h-[280px] flex items-center justify-center mr-2 relative">
            {selectedAvatar ? (
              <img src={selectedAvatar} alt="User Avatar" className="rounded-full w-full h-full object-cover" />
            ) : (
              <img src="/images/avatar2.avif" alt="Default Avatar" className="rounded-full w-full h-full object-cover" />
            )}
            {isEditing && (
              <AiOutlineEdit
                size={30}
              ...
              <div className="grid grid-cols-3 gap-2">
                {avatars.map((avatar, index) => (
                  <img
                    ...
                    src={avatar}
                    alt={`Avatar ${index + 1}`}
                    className={`w-16 h-16 rounded-full cursor-pointer ${selectedAvatar === avatar ? "border border-green-500" : ""}`}
                    onClick={() => {
